...
| --- | --- | --- | --- | --- |
| P1 | **Keep the two matching layers explicit.** The atlas reports 3,763/3,763 SAB-linked patches assigned to source-image groups; the relationship table reports 3,086 rows matched to public NDB-UFES origin images plus 677 rows without a public NDB-UFES match. | `results/phase0/validated_linkage/validated_linkage_summary.json`; `results/phase3/current_thesis_batches/relationship_update_summary.json` | Use “SAB/atlas source-image linkage” for all 3,763 patches and “public NDB-UFES origin matching” for the 3,086/677 split. Do not collapse them into one count. | **Documented — no longer an unexplained contradiction** |
...
| Source roles | Factsheet, data dictionary, public source-image index, and schema describe the public NDB-UFES + SAB versus SAB-only distinction. | No v1.0.0 field-level gap is known. | Keep the schema synchronized when the atlas inventory changes. |
