...
This public, sanitized package contains the two final thesis experiment split tables, the public validated source-image index, canonical stored-run evidence, summary statistics, and result figures.

- Experiment 1 is the released patch-level reference split.
- Experiment 2 is the patient-first grouped, lower-contamination-risk split.
- No checkpoints, raw MLflow storage, private SAB provenance, direct patient/lesion identifiers, private crosswalks, or local paths are included.

The source NDB-UFES dataset and P-NDB-UFES patch dataset remain separate works and must be cited from their original records. The documentation and derived release tables in this bundle are provided under CC BY 4.0; repository code is GPL-3.0-only.
"""
